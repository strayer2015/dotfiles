...
syntax match svDefine "`[0-9A-Z]*\(_\)*[0-9A-Z]*\>"
...
syntax match svNumber "'\<\([Hh][0-9a-fA-FZzXx]*\|[Dd][0-9ZzXx]*\|[Bb][01XxZz]*\)\>"
"syntax match svNumber "\<\([A-Z]*\)\+\(_\([A-Z]*\)\)\?\>"
syn match   svNumber      "\<[0-9][0-9_\.]\=\([fpnum]\|\)s\>"
syn match   svNumber      "\<[0-9][0-9_\.]\=step\>"

syntax match svNumber "\<[A-Z0-9]*\>"
syntax match svNumber "\<[A-Z0-9]*\(_[A-Z0-9]*\)*\>"
